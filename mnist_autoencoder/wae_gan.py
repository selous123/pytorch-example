...
trainset = MNIST(root='~/dataset/mnist',
                 train=True,
                 transform=transforms.ToTensor(),
                 download=True)

testset = MNIST(root='~/dataset/mnist',
                 train=False,
                 transform=transforms.ToTensor(),
                 download=True)
...
for epoch in range(args.epochs):
    ...
    if (epoch + 1) % 1 == 0:
        ...
        if not os.path.isdir('./wae/reconst_images'):
            os.makedirs('wae/reconst_images')

        save_image(test_data[0].view(batch_size, 1, 28, 28), './wae/reconst_images/wae_gan_input.png')
        save_image(reconst.data, './wae/reconst_images/wae_gan_images_%d.png' % (epoch + 1))

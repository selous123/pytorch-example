...
def d_loss(fake_logits,real_logits):
    loss = torch.mean(torch.log(real_logits)-torch.log(1-fake_logits))
    return loss;
def g_loss(fake_logits):
    loss = -torch.mean(torch.log(fake_logits))
    return loss
...
def visualize_result(fake_x,epoch):
    ...
    for i in range(h):
        for j in range(h):
            sum_a[i*28:(i+1)*28,j*28:(j+1)*28] = fake_images[i*10+j,:,:];
...
def train(d_net,g_net):
    ...
    torch.save(g_net.state_dict(),conf.result_directory+"gnet.pkl");
    torch.save(d_net.state_dict(),conf.result_directory+"dnet.pkl");

...
root_dir = "/home/lrh/program/git/pytorch-example/p-gan/data_small/cifar/image.npy"
...
nz = 100
g_steps = 5
result_directory = "result_smallcifar_1017_direct_all600data_onebatch_0.05LAMBDA"
batch_size = 600
bn = True
lr = 3e-4
beta = (0.5,0.999)
...
cuda = True
load = True
load_gnet_directory = "/home/lrh/program/git/pytorch-example/p-gan/result_smallcifar_1017_direct_all600data_onebatch_0.05LAMBDA/gnet_9000.pkl"
...
def loss_fn(real_x,fake_x,isopt=True,pi=None):
    ...
    if isopt:
        ...
        pi = utils.Hungarian(batch_size,c);
        pi = torch.from_numpy(pi).float().cuda()
        #print pi[0,:]
    #assert(type(pi)!=Nonetype)

    # pi_cvx = utils.simplexCVX(batch_size,c)
    # pi_cvx = torch.from_numpy(pi_cvx).float().cuda()
    # loss_cvx = torch.sum(torch.mul(cost_matrix,pi))
...
def train(g_net):
    ...
    for i,data in enumerate(dataloader,0):
        ...
        % (result_directory),normalize=True,nrow=20)
        break;
